...
classdef ISAMLocalizationSystem < minislam.localization.VehicleLocalizationSystem
    ...
    methods(Access = public)
        ...
        function [x, P] = getCurrentMeanAndCovarianceEstimate(this)
            ...
            try
                ...
                % strange way relative to global coordinates. Empirically,
                % it seems that the covariance is expressed in the
                % coordinate frame of the platform and has to be rotated
                % into the global coordinate frame. Unfortunately, GTSAM provides NO
                % documentation about this, none of the examples provide
                % any insight on the matter, and nobody answers the mailing
                % list :(
                theta=x.theta;
    ...
    methods(Access = protected)
        ...
        function addNewValue(this, newKey, initialValue)
            this.newValues.insert(newKey, initialValue);
            this.mergeRequiredIntoISAMRequired = true;
        end
        
        function addNewFactorAndValue(this, newKey, newFactor, initialValue)
            this.newFactors.add(newFactor);
            this.newValues.insert(newKey, initialValue);
            this.mergeRequiredIntoISAMRequired = true;
        end
        
        % These actually should be called "values" rather than "variables"
        function addNewVariable(this, newKey, initialValue)
            this.addNewValue(newKey, initialValue);
        end
        
        function addNewFactorAndVariable(this, newKey, newFactor, initialValue)
            this.addNewFactorAndValue(newKey, newFactor, initialValue);
        end
        
        % This method does two things. First, it creates a new vehicle pose
        % associated with the current time. Second, it generates the
        % factor to link it. 
        ...
        function advanceToEventTime(this, time)            
            ...
            % Nothing to do if it's really close to the last time, or the
            % incremental effects of the odometry are really small.
            if ((isempty(this.u) == false) && (norm(this.u * dT) < 1e-3))
                return
            end
            ...
            this.addNewFactorAndValue(newVehiclePoseKey, newOdometryFactor, newVehiclePose);
